/**
 * PudimMod — tradução do chat na sessão de jogo.
 *
 ...
 *
 * A tradução em si vem da ponte em gui/common/pudim_translate.js, que conversa
...
function pudim_TrRedesenharLinha(linha)
{
	if (linha.pudimEstado == "traduzido")
	{
		...
		linha.tooltip = pudim_T("chat.tr.original") + " " + linha.pudimTextoLimpo + "\n" +
			pudim_T("chat.tr.back");
	}
	else if (linha.pudimEstado == "esperando")
	{
		linha.text = linha.pudimOriginal + " " +
			pudim_TrPintar("(" + pudim_T("chat.tr.working") + ")", PUDIM_TR_COR_AVISO);
		linha.tooltip = pudim_T("chat.tr.sent");
	}
	else if (linha.pudimEstado == "semTradutor")
	{
		linha.text = linha.pudimOriginal;
		linha.tooltip = pudim_T("chat.tr.offline") + "\n" + pudim_T("chat.tr.offlineHow");
	}
	else
	{
		linha.text = linha.pudimOriginal;
		linha.tooltip = pudim_T("chat.tr.click");
	}
}
...
{
	try {
		return Engine.ConfigDB_GetValue("user", "pudim.chat.autotranslate") == "true";
	} catch (e) {
		return false;
	}
}
...
function pudim_TrAlternarTraduzirTudo()
{
	...
	Engine.ConfigDB_CreateAndSaveValue("user", "pudim.chat.autotranslate", String(novo));
...
pudim_patchApplyN("init", function(target, that, args)
{
	...
	try {
		...
		pudim_patchApplyN(ChatOverlay.prototype, "onChatMessage", function(alvo, esse, argumentos)
		{
			...
			if (msg && msg.type == "message" && chatMessage && chatMessage.text)
				try {
					...
					warn("PudimMod: falha ao preparar a tradução da fala: " + e);
				}

			return alvo.apply(esse, argumentos);
		});

		pudim_patchApplyN(ChatHistory.prototype, "displayChatHistory", function(alvo, esse, argumentos)
		{
			...
			try {
				...
				warn("PudimMod: falha ao traduzir o histórico do chat: " + e);
			}
		...
		warn("PudimMod: tradução do chat não pôde ser ligada: " + e);
	}
